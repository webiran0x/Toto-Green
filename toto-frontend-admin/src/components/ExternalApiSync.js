...
function ExternalApiSync() { // 'token' و 'API_BASE_URL' از پراپس حذف شدند
  ...
  const handleSyncGames = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        '/external/sync-games', // '/api' از ابتدای مسیر حذف شد
        {},
      ...
      console.error('Error syncing games:', err.response?.data || err.message); // لاگ برای اشکال‌زدایی
    } finally {
  ...
  const handleSyncResults = async () => {
    ...
    try {
      ...
      // کوکی‌ها به خاطر axios.defaults.withCredentials = true ارسال می‌شوند.
      const res = await axios.post(
        '/external/sync-results', // '/api' از ابتدای مسیر حذف شد
        {},
        // نیازی به هدر Authorization نیست
      );
      setMessage(res.data.message);
      ...
      console.error('Error syncing results:', err.response?.data || err.message); // لاگ برای اشکال‌زدایی
    } finally {
