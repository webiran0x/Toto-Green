...
function OpenTotoGames() {
  ...
  return (
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md transition-colors duration-300">
      <h2 className="text-3xl font-extrabold text-gray-800 dark:text-white mb-6 text-center">{t('active_toto_games')}</h2>
      ...
      {openGames.length === 0 ? (
        <p className="text-gray-600 dark:text-gray-400 text-center py-4">{t('no_active_games')}</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {openGames.map((game) => (
            <div 
              key={game._id} 
              className="bg-blue-50 dark:bg-gray-700 p-6 rounded-xl shadow-lg border border-blue-200 dark:border-gray-600 flex flex-col transform transition-transform duration-300 hover:scale-[1.02] animate-fadeIn transition-colors duration-300"
            >
              <h3 className="text-xl font-bold text-blue-800 dark:text-blue-200 mb-2">{game.name}</h3>
              <p className="text-gray-700 dark:text-gray-300 text-sm flex items-center mb-1">
                <ClockIcon className="h-4 w-4 mr-2 text-gray-600 dark:text-gray-400" />
                {t('deadline')}: {new Date(game.deadline).toLocaleString('fa-IR')}
              </p>
              <p className="text-gray-700 dark:text-gray-300 text-sm flex items-center mb-4">
              ...
              <button
                ...
                className="mt-auto bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline transition duration-300 ease-in-out transform hover:scale-105 flex items-center justify-center shadow-md"
              >
      ...
      {selectedGame && ( // فرم پیش‌بینی فقط در صورت انتخاب بازی نمایش داده می‌شود
        <div className="p-6 border border-blue-200 dark:border-gray-600 rounded-xl shadow-lg bg-blue-50 dark:bg-gray-700 animate-fadeIn transition-colors duration-300">
          <h3 className="text-2xl font-bold text-gray-800 dark:text-white mb-4 text-center">
            {t('predict_for')}: {selectedGame.name}
          </h3>
          <p className="text-xl font-bold text-blue-700 dark:text-blue-400 mb-6 text-center">
          ...
          <form onSubmit={handleSubmitPrediction}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {selectedGame.matches.map((match) => (
                <div
                  key={match._id}
                  className="bg-white dark:bg-gray-800 p-5 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 transform transition-transform duration-200 hover:scale-[1.02] transition-colors duration-300"
                >
                  ...
                  <div className="flex flex-col space-y-2">
                    <label className="inline-flex items-center cursor-pointer bg-blue-100 dark:bg-blue-900 p-2 rounded-md hover:bg-blue-200 dark:hover:bg-blue-800 transition duration-150 transition-colors duration-300">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="ml-2 text-gray-700 dark:text-gray-200">{t('home_win_abbr', { homeTeam: match.homeTeam })}</span>
                    </label>
                    <label className="inline-flex items-center cursor-pointer bg-blue-100 dark:bg-blue-900 p-2 rounded-md hover:bg-blue-200 dark:hover:bg-blue-800 transition duration-150 transition-colors duration-300">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="ml-2 text-gray-700 dark:text-gray-200">{t('draw_abbr')}</span>
                    </label>
                    <label className="inline-flex items-center cursor-pointer bg-blue-100 dark:bg-blue-900 p-2 rounded-md hover:bg-blue-200 dark:hover:bg-blue-800 transition duration-150 transition-colors duration-300">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="ml-2 text-gray-700 dark:text-gray-200">{t('away_win_abbr', { awayTeam: match.awayTeam })}</span>
                    </label>
                  </div>
                </div>
              ))}
            ...
            <div className="mt-8 flex justify-center">
              <button
                type="submit"
                disabled={submitting}
                className="bg-gradient-to-r from-green-600 to-teal-700 hover:from-green-700 hover:to-teal-800 dark:from-green-700 dark:to-teal-800 dark:hover:from-green-800 dark:hover:to-teal-900 text-white font-bold py-3 px-8 rounded-lg text-xl focus:outline-none focus:shadow-outline transition duration-300 ease-in-out transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
              >
      ...
      {showErrorModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-md w-full p-6 transition-colors duration-300">
            <h3 className="text-xl font-bold mb-4 text-red-600 dark:text-red-400">{t('error')}</h3>
            <p className="mb-6 text-gray-700 dark:text-gray-300 whitespace-pre-line">{modalErrorMessage}</p> 
