// toto-landing/src/pages/FAQPage.js
// کامپوننت صفحه پرسش و پاسخ (FAQ) با Tailwind CSS و پشتیبانی از حالت تیره
// بر اساس طراحی ارائه شده و استفاده از داده‌های واقعی پلتفرم توتو
...
import {
  ...
  PhoneIcon,
  EnvelopeIcon,
  CreditCardIcon, // برای Payment (معادل tabler-icon-credit-card)
  TruckIcon, // برای Delivery (معادل tabler-icon-truck)
  ArrowPathIcon // برای Cancellation & Return (معادل tabler-icon-arrow-rotary-first-left)
...
const faqData = [
  ...
  },
  {
    ...
    icon: CurrencyDollarIcon, // Heroicons equivalent of Payment icon
...
function FAQPage({ currentTheme, toggleTheme }) { // currentTheme و toggleTheme را از App.js دریافت می‌کنیم
  const { t } = useLanguage();
  ...
  return (
    <div className="bg-clr-surface-a0 min-h-screen flex flex-col font-iranyekan">
      ...
      <main className="flex-grow container mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="bg-clr-surface-a0 p-6 sm:p-8 rounded-2xl shadow-xl w-full max-w-3xl text-center mx-auto mb-8 transition-colors duration-300">
          <p className="text-clr-dark-a0 dark:text-clr-light-a0 mb-2">{t('hello_how_can_we_help')}</p>
          <div className="relative w-full max-w-md mx-auto">
            ...
            <input
              ...
              className="block w-full rounded-lg border-clr-surface-a30 shadow-sm focus:border-clr-primary-a0 focus:ring-clr-primary-a0 p-3 pl-10 rtl:pr-10 text-lg bg-clr-surface-a10 text-clr-dark-a0 dark:text-clr-light-a0 transition duration-200 ease-in-out placeholder-clr-surface-a40 dark:placeholder-clr-surface-a50"
              value={searchTerm}
            ...
            <div className="flex lg:flex-col space-x-2 lg:space-x-0 lg:space-y-2 overflow-x-auto lg:overflow-x-hidden pb-2 lg:pb-0">
              {faqData.map(category => {
                ...
                return (
                  ...
                  >
                    <IconComponent className="h-5 w-5 mr-2 rtl:ml-2 rtl:mr-0" /> {/* rtl:ml-2 rtl:mr-0 برای RTL */}
                    {t(category.titleKey)}
                  </button>
                );
              })}
              ...
              activeTab === category.category && (
                <div key={category.category}>
                  <div className="flex items-center mb-6">
                    <div className="p-3 bg-clr-primary-a50 rounded-full mr-4 rtl:ml-4 rtl:mr-0"> {/* mr-4 rtl:ml-4 */}
                      <category.icon className="h-8 w-8 text-clr-primary-a0" />
                    </div>
                    <div>
                      <h3 className="text-2xl font-bold text-clr-dark-a0 dark:text-clr-light-a0 mb-1">{t(category.titleKey)}</h3>
                      ...
                      <div className="space-y-4">
                          {filteredQuestions.map(q => (
                              <div key={q.qKey} className="border border-clr-surface-a20 rounded-lg overflow-hidden shadow-sm transition-colors duration-300">
                                  <button
                                      className="flex justify-between items-center w-full p-4 bg-clr-surface-a10 hover:bg-clr-surface-a20 focus:outline-none transition-colors duration-200 text-left rtl:text-right" // text-left rtl:text-right
                                      onClick={() => handleAccordionToggle(q.qKey)}
                                  >
                                      <span className="font-semibold text-clr-dark-a0 dark:text-clr-light-a0">{t(q.qKey)}</span>
        ...
        <div className="bg-clr-surface-a0 p-6 sm:p-8 rounded-2xl shadow-xl w-full max-w-3xl text-center mx-auto mt-8 transition-colors duration-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col items-center p-4 bg-clr-surface-a10 rounded-lg shadow-sm border border-clr-surface-a20 transition-colors duration-300">
              <div className="p-3 bg-clr-primary-a50 rounded-full mb-3">
